...
<div class="space-y-6">
    ...
    @endif
    {{-- Notifikasi Info (dari redirect tagEdit) --}}
    @if (session('info'))
    ...
    @endif
    @if ($errors->any())
        {{-- Menampilkan error validasi dari proses Simpan Tag Baru --}}
    ...
    <div class="bg-bg-dark light:bg-white rounded-xl p-6 shadow-xl border border-sienna/50 light:border-gray-200 mb-6">
        ...
        <form action="{{ route('admin.tags.store') }}" method="POST" class="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4 items-end">
            ...
            <div class="flex-grow w-full">
                ...
                <input type="text" id="new_tag_name" name="name" required placeholder="Contoh: Drone, IoT, Hilirisasi"
    ...
    <div class="bg-bg-dark light:bg-white rounded-xl shadow-xl overflow-hidden border border-sienna/50 light:border-gray-200">
        <div class="p-6">
            <h3 class="text-xl font-semibold text-cream-text light:text-light-text mb-4">Tag Tersedia (Edit Langsung)</h3>
                ...
                <table class="min-w-full divide-y divide-sienna/30 light:divide-gray-200">
                    <thead class="bg-sienna/20 light:bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-cream-text/80 light:text-gray-600 uppercase tracking-wider">Nama Tag (Dapat Diedit)</th>
...
{{-- SCRIPT JAVASCRIPT UNTUK MENANGANI INLINE EDIT --}}
<script>
    ...
    /**
     * Fungsi untuk mengirim permintaan PUT (Update) saat nilai input tag berubah.
     */
    function handleTagUpdate(inputElement) {
        const tagId = inputElement.dataset.tagId;
        ...
        .then(data => {
            ...
            if (row) {
                ...
                setTimeout(() => {
                    row.classList.remove('bg-green-700/30');
                }, 1500);
            }
        })
        .catch(error => {
            ...
            if (row) {
                ...
                setTimeout(() => {
                    row.classList.remove('bg-red-700/30');
                }, 2000);
            }
        });
    }
