                        ...
                        @foreach ($categories as $category)
                            <tr id="category-row-{{ $category->id }}" class="hover:bg-sienna/10 light:hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                    ...
                                    <form action="{{ route('editor.categories.destroy', $category) }}"
                                             method="POST"
                                             class="inline-block"
...
<script>
    ...
    function handleCategoryUpdate(inputElement) {
        ...
        // PERBAIKAN URL AJAX: Menggunakan prefix 'editor'
        const url = `{{ url('editor/masters/categories') }}/${categoryId}`;
        ...
        .then(response => {
            if (!response.ok) {
                return response.json().then(data => {
                    ...
                    // Mendapatkan pesan error validasi spesifik
